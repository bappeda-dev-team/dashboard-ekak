...
const PERATURAN_URL = process.env.NEXT_PUBLIC_PERATURAN_URL || "#";
...
export function UrlProvider({ children }: Readonly<{ children: React.ReactNode; }>) {
    return (
        <UrlContext.Provider value={{ authUrl: AUTH_URL, perencanaanUrl: PERENCANAAN_URL, realisasiUrl: REALISASI_URL, tppUrl: TPP_URL, laporanUrl: LAPORAN_URL, manriskUrl: MANRISK_URL, peraturanUrl: PERATURAN_URL }}>
            {children}
        </UrlContext.Provider>
    );
}
